...
public class WolfSchedulerTest {
	...
	public void testAddCourse() throws ConflictException {
		...
		//Attempt to add a course that has conflict with the one already in the schedule
		try{
			ws.addCourse("CSC230", "001");
		} catch(IllegalArgumentException e) {
	...
	public void testAddEvent() throws ConflictException {
		...
		//Attempt to add an event that conflicts with the one already in the schedule
		try {
			ws.addEvent("Lunch", "MW", 730, 830, 1, "Lunch with friend");
		} catch(IllegalArgumentException e) {
		...
		//Attempt to add an event that conflicts with the one already in the schedule
		try {
			ws.addEvent("Fair", "MTW", 830, 930, 1, "Career fair");
		} catch(IllegalArgumentException e) {
			assertEquals("The event cannot be added due to a conflict.", e.getMessage());
		}

		//Attempt to add an event that overlaps it's end time with the start time of 
		//the one already in the schedule
		try {
			ws.addEvent("Festival", "MWF", 730, 800, 1, "Spring fest");
		} catch(IllegalArgumentException e) {
			assertEquals("The event cannot be added due to a conflict.", e.getMessage());
		}
	}
