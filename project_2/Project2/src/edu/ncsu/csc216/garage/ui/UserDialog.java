...
public class UserDialog extends JDialog {
	...
	/** Vehicle data fields and combo labels */
	...
	/**
	 * dialog box for updated the tiered
	 * 
	 * @param v
	 *            vehicle to be edited
	 */
	public UserDialog(Tiered v) {
		super((java.awt.Frame) null, TITLE_BAR_EDIT, true);

		Vehicle vehicle = (Vehicle) v;

		vehicleData[0] = vehicle.getName();
		vehicleData[1] = vehicle.getLicense();

		cboTier.setSelectedItem(false);

		if (vehicle.toString().substring(0, 1).equalsIgnoreCase("E")) {
	...
	/**
	 * sets layout and adds panels/widgets for edit screen
	 */
	private void setUpEditGeometry() {
		pnlBtn.setLayout(new FlowLayout());
		pnlBtn.add(btnSubmit);
		pnlBtn.add(btnCancel);
	...
	/**
	 * creates the grid to lay out the labels/ widgets
	 */
	private void setUpEditInfo() {
		...
		for (int k = 0; k < 3; k++) {
			...
			if (k < 2) {
				compInfo[k] = new JTextField(String.format("%-20s", vehicleData[k]));
				compInfo[k].setEnabled(false);
			} else {
				compInfo[k] = cboTier;
			}
	 ...
	 *             if a Car cannot be created from the given information
	 */
